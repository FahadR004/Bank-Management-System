            ...
            while True:
                try:
                    if last_date_paid == 0 and initial_date_diff == 0:  # Meaning loan was just taken.
                        ...
                        print("---------------\n")
                        break
                    months = (cur_date.year - date_of_loan.year) * 12 + (cur_date.month - date_of_loan.month)
                    dep_loan = 0  # Variable Initialization to not raise error in else block.
                    initial_loan = months * self.loan_per_month  # Same here
                    if last_date_paid == 0 and initial_date_diff != 0:
                        if months == 1:
                            ...
                            print(f"Your initial total loans were Rs. {initial_loan} but as a result of extra "
                                  f"interest have been increased to Rs. {total_loan}")
                            loan_pay = int(input("Please duly enter the correct amount of money here: "))
                            assert loan_pay == total_loan
                    elif last_date_paid != 0:
                        last_date_paid = datetime.datetime.strptime(str(last_date_paid), "%Y-%m-%d")
                        if last_date_paid.month == cur_date.month:
                            print("You have paid the loan for this month!")
                            print("-------------------------------------\n")
                            time.sleep(0.1)
                            break
